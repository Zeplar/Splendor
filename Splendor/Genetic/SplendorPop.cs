...
//
// Edited by Joshua Hepworth 2015
// All instances of "Random" have been changed to "Splendor.Random" to conserve the seed.
// All instances of evaluation have been removed.
// Some variable accessibility has been changed.
...
namespace Splendor.Genetic
{
    ...
    public class SplendorPop
    {
        ...
        public List<IChromosome> population = new List<IChromosome>();
        protected int size;
        protected double randomSelectionPortion = 0.0;
        protected bool autoShuffling = false;

        // population parameters
        protected double crossoverRate = 0.75;
        protected double mutationRate = 0.10;
        ...
        public virtual void Crossover()
        {
            ...
            {
                Debug.Assert(population.Count >= size, "" + i);

                // generate next random number and check if we need to do crossover
                if (Splendor.random.NextDouble() <= crossoverRate)
                {
        ...
        public virtual void Mutate()
        {
            ...
            for (int i = 0; i < size; i++)
            {
                ...
                if (Splendor.random.NextDouble() <= mutationRate)
                {
        ...
        public virtual void Selection()
        {
            ...
            Debug.Assert(population.Count == size, "Selection failed to trim population");
        }
        ...
        public void Shuffle()
        {
            ...
            while (size > 0)
            {
                int i = Splendor.random.Next(size);
